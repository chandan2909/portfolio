        ...
        <>
            <section id="skills" aria-labelledby="skills-heading" className="py-20 animate-on-scroll">
                <div className="container mx-auto max-w-6xl px-8">
                    ...
                    {loading ? (
                        <div className="text-center py-20 text-gray-500 font-bold uppercase tracking-widest">Loading skills...</div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="list">
                            {technicalSkills.map((skill) => (
                                <div key={skill._id || skill.name} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-500 group cursor-pointer" role="listitem">
                                    <div className="flex items-center mb-6">
                                        <div className="bg-black text-white rounded-full p-3 mr-4 group-hover:scale-110 transition-transform font-bold w-12 h-12 flex items-center justify-center">
                                            {skill.name.charAt(0)}
                                        </div>
